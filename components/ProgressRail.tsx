  ...
  return (
    <nav aria-label="Progress" className="px-1">
      <div className="relative">
        ...
        <div
          className="absolute left-0 top-5 mx-[12.5%] h-1 rounded-full bg-accent-400/70 transition-[width] duration-500 ease-out"
              ...
              <li key={step.id} className="flex flex-1 flex-col items-center gap-2">
                <button
                  ...
                  className={`relative flex h-11 w-11 items-center justify-center rounded-full border-2 bg-surface transition-all duration-300 ${
                    isActive
                      ? 'scale-110 border-accent-400 text-white shadow-[0_4px_14px_rgba(132,204,22,0.2)]'
                      : isComplete
                        ? 'border-accent-400 text-accent-500'
                        : 'border-[var(--step-inactive)] text-[var(--text-muted)]'
                  } ${
                    isReachable && onStepClick && !isActive
                      ? 'cursor-pointer hover:border-accent-400/60'
                      : 'cursor-default'
                  }`}
