...
/**
 * Server-side fallback for filled text fields. The primary replacement now
 * happens client-side (fillTextFieldsClient.ts) which renders affected pages
 * as images with the text baked in. This function only runs if the server
 * receives filledTextFields directly (e.g. from older clients).
 */
